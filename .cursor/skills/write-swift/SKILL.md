---
name: write-swift
description: How to write modern Swift well (value types, concurrency, generics, API design, performance). Vendored from emilkowalski/skills with an Inboxies overlay — this app is Swift 5.10 / iOS 17, not Swift 6.3. Use when writing or reviewing Swift under ios/. For SwiftUI look-and-feel use inboxies-ios-ui instead.
---

# Write Swift

## Inboxies overlay (read this first)

This file is vendored from [emilkowalski/skills write-swift](https://github.com/emilkowalski/skills) (MIT). The language guidance is excellent; **the toolchain notes are not this repo**.

- **This target is Swift 5.10 / iOS 17** (`ios/AgenticInbox/project.yml`). Xcode project also records `SWIFT_VERSION = 5.0` in places — do not treat that as a prompt to upgrade.
- **Do not migrate** to Swift 6.3/6.4, `@concurrent`, Swift Testing, macros, or approachable-concurrency defaults unless the user explicitly asks.
- App architecture: `@Observable` classes (`AppModel`, `AuthStore`) injected with `.environment`. That is the existing pattern — not a reason to rewrite stores as value types.
- UI work belongs in `inboxies-ios-ui`. This skill is language/API/concurrency/performance, not Notion chrome.
- Prefer `async/await` + `URLSession` as `APIClient` already does. Stay on the main actor for UI. Don't introduce actors for a view change.
- Skip sections that require Swift 6.2+ language mode if they wouldn't compile on 5.10.
...
- **Prefer has-a to is-a.** If only some of a protocol's operations make sense for your type, don't refine it — wrap it in a generic struct and expose exactly the API you mean. (`GeometricVector ` rather than `GeometricVector: SIMD`.)
