...
description: Reverse-lookup glossary that turns a vague motion description into the exact term. Use when the user asks "what's it called when…" or describes a motion effect without knowing its name. For naming an effect, not designing or building one. Inboxies overlay maps terms onto SwiftUI/Compose APIs already in the apps.
---

# Animation Vocabulary

Vendored from [emilkowalski/skills](https://github.com/emilkowalski/skills) (MIT). Copyright (c) 2026 Emil Kowalski. See `.cursor/skills/NOTICE`.

## Inboxies overlay

This glossary is platform-agnostic. When naming something **in these apps**, prefer the in-repo mapping:

| Term | Inboxies iOS | Inboxies Android |
| --- | --- | --- |
| Shared element / zoom | iOS 18 `navigationTransition(.zoom)` + `matchedTransitionSource` (Ask AI, Compose, email actions) | not shipped — don't invent a lib |
| Materials / glass | `.liquidGlass` / `glassEffect` (iOS 26) / ultraThinMaterial fallback | `Modifier.liquidGlass` frost + hairline |
| Direction-aware transition | folder tab ±28 / ±18 offset | 28.dp / 18.dp `AnimatedContent` slide |
| Rubber-banding | system list overscroll; custom drags on chat/compose | same |
| Skeleton / pulse | `.skeletonPulse` 0.95s | list skeleton tween 950ms |
| Spring | `.spring(response: 0.32, dampingFraction: 0.86)` | `dampingRatio 0.86` + `StiffnessMediumLow` |
| Swipe to dismiss | chat / compose dock / mail swipe actions | same |
| Press feedback | row opacity 0.55 | row alpha 0.55 |

For **building** the motion, use `animate`. For **reviewing**, `review-animations`.
...
- **Page transition** — An animation that plays when navigating from a page or route to another.
